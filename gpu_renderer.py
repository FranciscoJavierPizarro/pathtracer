...
if CUDA_AVAILABLE:
    ...
    def trace_ray_gpu(origin, direction, light_pos,
                    ...
                    #   triangle_uv1s, triangle_uv2s, triangle_uv3s,  ################################################################# NO SE SI SE USAN?
        ...
        for bounce in range(max_depth + 1):
            ...
            for i in range(triangle_count):
                ...
                pt2 = (triangle_pt2s[i, 0], triangle_pt2s[i, 1], triangle_pt2s[i, 2])
                pt3 = (triangle_pt3s[i, 0], triangle_pt3s[i, 1], triangle_pt3s[i, 2])
                # uv1 = (triangle_uv1s[i, 0], triangle_uv1s[i, 1], triangle_uv1s[i, 2])
                # uv2 = (triangle_uv2s[i, 0], triangle_uv2s[i, 1], triangle_uv2s[i, 2])
                # uv3 = (triangle_uv3s[i, 0], triangle_uv3s[i, 1], triangle_uv3s[i, 2])
            ...
            # Agregar contribución de luz directa (reducida para evitar sobre-exposición)
            kd = hit_material[0]
            direct_contribution = (
                throughput[0] * hit_color[0] * kd * direct_light_intensity * 0.5,  # Factor 0.5 para balancear
                throughput[1] * hit_color[1] * kd * direct_light_intensity * 0.5,
                throughput[2] * hit_color[2] * kd * direct_light_intensity * 0.5
            )
            ...
            if rand_val < prob_kd:
                ...
                # BRDF Lambertiana con muestreo uniforme corregida
                # PDF = 1/(2*pi), BRDF = albedo/pi
                # Factor = BRDF * cos(theta) / PDF = (albedo/pi) * cos(theta) / (1/(2*pi))
                cos_theta = dot_gpu(bounce_dir, hit_normal)
                if cos_theta <= 0.0:
                    break  # Rayo por debajo de la superficie
                
                # Factor corregido: 2 * cos(theta) / pi (no 2.0 * cos_theta)
                brdf_factor = 2.0 * cos_theta / 3.14159265359
                
                # Terminación rusa progresiva para evitar sobre-iluminación
                russian_prob = 1.0
                if bounce > 2:
                    # Reducir probabilidad gradualmente después del rebote 2
                    russian_prob = max(0.1, 1.0 - (bounce - 2) * 0.15)
                    if random_uniform(state) > russian_prob:
                        break
                    brdf_factor /= russian_prob  # Compensar por terminación rusa
    ...
    def render_kernel(image, width, height, origin, lower_left_corner,
                      ...
                      plane_points, plane_normals, plane_colors, plane_materials, plane_iors, plane_count,
                      triangle_pt1s, triangle_pt2s, triangle_pt3s, 
                    #   triangle_uv1s, triangle_uv2s, triangle_uv3s,
                      triangle_colors, triangle_materials, triangle_iors, triangle_count, 
...
class GPURenderer:
    ...
    def __init__(self, samples_per_pixel=4, max_bounces=5):  # Valores más balanceados
    ...
    def render(self, scene, camera, width, height):
        ...
        # Preparar los datos de triangulos
        triangle_pt1s = np.array([p.pt1 for p in scene.triangles], dtype=np.float32)
        triangle_pt2s = np.array([p.pt2 for p in scene.triangles], dtype=np.float32)
        triangle_pt3s = np.array([p.pt3 for p in scene.triangles], dtype=np.float32)
        # triangle_uv1s = np.array([p.uv1 for p in scene.triangles], dtype=np.float32)
        # triangle_uv2s = np.array([p.uv2 for p in scene.triangles], dtype=np.float32)
        # triangle_uv3s = np.array([p.uv3 for p in scene.triangles], dtype=np.float32)
        triangle_colors = np.array([p.color for p in scene.triangles], dtype=np.float32)
        triangle_materials = np.array([p.material for p in scene.triangles], dtype=np.float32)
        triangle_iors = np.array([p.ior for p in scene.triangles], dtype=np.float32)
        ...
        render_kernel[blocks_per_grid, threads_per_block](
            ...
            triangle_pt1s, triangle_pt2s, triangle_pt3s, 
            # triangle_uv1s, triangle_uv2s, triangle_uv3s,
            triangle_colors, triangle_materials, triangle_iors, len(scene.triangles), 
